//! The procurement reports' data layer, shared by the JSON endpoints and the
//! report engine — so the API and the PDF can never disagree about a number.
//!
//! Each view is the shape one report needs; [`ProcurementQueries`] builds them
//! from the posted paper trail alone. Kept apart from the reports themselves:
//! this is the arithmetic, they are the presentation, and the two change for
//! different reasons.
