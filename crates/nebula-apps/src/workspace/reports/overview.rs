...
//!
//! The reference sheet for the reporting engine — headings, styled text, KPI
//! tiles, every chart kind, side-by-side blocks, a boxed table group, a
//! progress bar, a callout, lists, a QR/barcode pair and a signature band.
//! It uses only the framework company datasource, so it needs no business data
//! and renders on a bare tenant.

use nebula::{
    Callout, CalloutStyle, Chart, ChartKind, Column, Group, KeyValue, Metric, Progress, Report,
    ReportData, ReportDefinition, ReportFormat, Result, Series, Signature, SpaceSize, Symbology,
